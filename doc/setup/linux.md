...
Мы подготовили `Docker`-образ для работы с `Xv6`. Там установлено все необходимое: виртуальная машина `qemu`, компилятор `GCC` под `RISC-V`, `make` для автоматизации сборки проекта и прочие мелочи.
...
Также вы можете собрать образ самостоятельно.
