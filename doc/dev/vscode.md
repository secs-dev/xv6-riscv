...
Запустится `qemu` и будет дожидаться подключения дебаггера. Для запуска `gdb` нажмите `F5`.
