# Задание. Введение в Xv6

## Введение

Эта ЛР знакомит с `Xv6` и её системными вызовами: вы напишете user-space код для обмена данными между процессами через pipes и реализуете собственный системный вызов для инспекции состояния процесса. Это фундамент для всех последующих работ — пользовательский и kernel-space интерфейс ядра.
...
4. Прочитать его в дочернем процессе, вывести `<child pid>: got <message>` и отправить сообщение `pong` в ответ.
...
- Функцию dump нужно также определить в заголовочном файле [kernel/defs.h](/kernel/defs.h).
...
Мы бы могли использовать системный вызов `dump`, чтобы написать собственный отладчик. Однако у него есть два недостатка. Во-первых, он выводит значения регистров на экран, и мы не можем обработать эти значения в user-space. Во-вторых, он позволяет узнать значения регистров только у текущего процесса, что делает невозможной отладку другого процесса. Напишем ещё один системный вызов, чтобы исправить эти недостатки — `dump2`.
...
Обратите внимание, что в целях безопасности регистры процесса могут смотреть только сам процесс и его предки.
