...
Наша цель — избавиться от этого, заменить аллокатор памяти и выделять файловые структуры динамически. Свой аллокатор писать не потребуется — [buddy allocator][1] уже написан в [kernel/buddy.c](/kernel/buddy.c). Остается инициализировать этот аллокатор — отредактировать базовый аллокатор [kernel/kalloc.c](/kernel/kalloc.c) (добавить вызовы функций из [kernel/buddy.c](/kernel/buddy.c)).
